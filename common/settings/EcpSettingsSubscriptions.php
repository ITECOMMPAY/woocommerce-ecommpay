...
class EcpSettingsSubscriptions extends EcpSettings {
	...
	public function __construct() {
		$this->id        = self::ID;
		$this->label_key = 'Subscriptions';
		$this->icon      = 'ecommpay.svg';
		$this->context   = self::CONTEXT;

		add_filter( EcpFilters::ECP_PREFIX_GET_SETTINGS . $this->id, array( $this, 'get_settings_subscriptions' ) );
	...
	public function get_settings_subscriptions(): array {
		...
		if ( ! ecp_subscription_is_active() || EcpSettingsGeneral::PURCHASE_TYPE_AUTH !== $purchaseTypeOption ) {
		...
		$settings = array(
			...
			array(
				self::FIELD_TITLE => $this->fieldText(
					'Payments only for the selected subscription type will be '
