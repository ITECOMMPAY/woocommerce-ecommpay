<?php
/**
 * ECOMMPAY Gateway Subscription API class.
 *
...
class EcpGatewayAPISubscription extends EcpGatewayAPI {
	...
	public function recurring( int $subscription_id, EcpGatewayOrder $order, int $amount = null ): EcpGatewayInfoResponse {
		...
		if ( ! class_exists( 'WC_Subscriptions_Order' ) ) {
			...
			throw new EcpGatewayAPIException( __( 'Woocommerce Subscription plugin is not available.', 'woo-ecommpay' ), EcpGatewayError::NOT_AVAILABLE );
		}
		...
		if ( ! $payment_method ) {
			throw new EcpGatewayAPIException( __( 'Payment method is not supported subscription.', 'woo-ecommpay' ), EcpGatewayError::NOT_IMPLEMENTED );
		}
	...
	final public function create_recurring_request_form_data( int $subscription_id, EcpGatewayOrder $order ): array {
		...
		$data['recurring']    = array( 'id' => $subscription_id );
